...
export fn _start() noreturn {
    const script: []const u8 = @embedFile("assets/terminal.lua");
    runScript(script);
    std.process.exit(0);
}

// Reutilizamos la lógica del intérprete (copiada simplificada o importada si refactorizamos)
// Por ahora, copiaremos la lógica para asegurar independencia rápida.
...
fn runScript(src: []const u8) void {
    ...
    while (it.next()) |line| {
        ...
        if (std.mem.startsWith(u8, trimmed, "print(")) {
             // ... parsing logic (simplified for brevity if needed)
        }
    }
}

fn processGraphicsCommand(line: []const u8) bool {
    // ... Copy of logic from wasm_app.zig ...
    // Para simplificar, solo implementamos lo que usan los scripts creados
    ...
    if (std.mem.startsWith(u8, line, "draw_text_at(")) {
        // Full parser needed? Let's assume standard format for now or copy full parser.
        // Copying full parser is safer.
...
// ... Parsers (Need to copy from wasm_app.zig to ensure it works) ...
...
fn parseDrawRect(line: []const u8) bool {
    ...
    const filled = std.mem.eql(u8, std.mem.trim(u8, f_str, "\""), "true"); // Lua sends string "true" in my script? checking script... yes "true".
